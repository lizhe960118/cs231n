...
def conv_relu_pool_forward(x, w, b, conv_param, max_pool_param):
    c, conv_cache = conv_forward_fast(x, w, b, conv_param)
    cr, relu_cache = relu_forward(c)
    out, max_pool_cache = max_pool_forward_fast(cr, max_pool_param)
    cache = (conv_cache, relu_cache, max_pool_cache)
    return out, cache

def conv_relu_pool_backward(dout, cache):
    conv_cache, relu_cache, max_pool_cache = cache
    dmp = max_pool_backward_fast(dout, max_pool_cache)
    da = relu_backward(dmp, relu_cache)
    dx, dw, db = conv_backward_fast(da, conv_cache)
    return dx, dw, db
